...
// @Router /exchange/handle [post]
func AcceptRejectExchangeRequest(w http.ResponseWriter, r *http.Request, db *sql.DB) {
	...
	err = db.QueryRow(`
		SELECT user_id, amount, fromcurrency, tocurrency 
		FROM requests 
		WHERE id = $1 AND status = 'pending'`, requestID).Scan(&clientUserID, &amount, &fromcurrency, &tocurrency)
	if err != nil {
	...
	err = exchange.Exchange(db, clientUserID, amount, fromcurrency, tocurrency)
	if err != nil {
